...
#saveRDS(participants_to_exclude$id,here::here("data/s1_grp_outlier_ids.rds"))
...
s2_agg_filtered <- s2_agg1 |>
  filter(!(id %in% participants_to_exclude$id))

#saveRDS(participants_to_exclude$id,here::here("data/s2_grp_outlier_ids.rds"))


mixed_model_abs_error <- lmer(log_abs_error ~ refClass + calc+ (1|id) + (1|state), data = s2_agg_filtered)
summary(mixed_model_abs_error)
# with p value
library(lmerTest)
mixed_model_abs_error <- lmer(log_abs_error ~ refClass + calc+ (1|id) + (1|state), data = s2_agg_filtered)
...
library(lmerTest)
mixed_model_abs_error <- lmer(log_abs_error ~ refClass*rounded*pct_goal + calc+ (1|id) + (1|state), data = s2_agg_filtered)
summary(mixed_model_abs_error)


ordinal_model_s2_logit <- brm(
  accuracy_level ~ (refClass*rounded*pct_goal)+calc + (1|id)+ (1|state),
  data = s2_agg_filtered,
  ...
  file = paste0(here::here("data/model_cache",'s2_logit_grpclean_calc_int.rds')) # Cache for efficiency
)
...
prob_exact <- posterior_preds[, , "Exact match"]
prob_minor <- posterior_preds[, , "0.01-2% error"]
prob_moderate <- posterior_preds[, , "2.01-15% error"]
prob_major <- posterior_preds[, , "Over 15% error"]

# Compute mean probabilities across all posterior samples
s2_agg_filtered <- s2_agg_filtered %>%
  mutate(
    prob_exact = colMeans(prob_exact),
    prob_minor = colMeans(prob_minor),
    prob_moderate = colMeans(prob_moderate),
    prob_major = colMeans(prob_major)
  )

emm_refClass_manual <- s2_agg_filtered %>%
  group_by(refClass) %>%
  summarise(
    mean_prob_exact = mean(prob_exact),
    mean_prob_minor = mean(prob_minor),
    mean_prob_moderate = mean(prob_moderate),
    mean_prob_major = mean(prob_major)
  )

print(emm_refClass_manual)


ggplot(emm_refClass_manual, aes(x = refClass)) +
  geom_bar(aes(y = mean_prob_exact), stat = "identity", fill = "steelblue") +
  geom_errorbar(aes(ymin = mean_prob_exact - 0.05, ymax = mean_prob_exact + 0.05), width = 0.2) +
  labs(title = "Estimated Marginal Means for Reference Class",
       x = "Reference Class",
       y = "Probability of Exact Match") +
  theme_minimal()
```
...
2    16 Texas  Massachusetts USD      R_3gNKA8IQr0uOSh3 TEX   Water Heating  34265 4396      plan2  6414     1     0.15
 ...
 s1 |> filter(state_avg==29046)
# A tibble: 2 × 21
# Groups:   id [1]
     id state1   state2 refClass calc  edu   modality state id2   Duration__in_seconds_ appliance family state_avg plan  value block
  <int> <chr>    <chr>  <chr>    <chr> <fct> <chr>    <chr> <chr>                 <dbl> <chr>      <dbl> <dbl+lbl> <fct> <dbl> <dbl>
1    12 Colorado Texas  kWh      Used… Grad… A        TEX   R_ab…                   644 Total kWh  29046 29046     plan1 29046     2
2    12 Colorado Texas  kWh      Used… Grad… A        TEX   R_ab…                   644 Total kWh  29046 29046     plan2 29046     2
ℹ 5 more variables: pct_goal <dbl>, change <dbl>, state_p_dif <dbl>, state_f_dif <dbl>, less_avg <lgl>
...
## Inspects counts for each distinct value of prior year family use, and state average
