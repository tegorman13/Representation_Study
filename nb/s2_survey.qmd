...
s2_agg4 <- s2_agg1 |> group_by(id,refClass,calc,els) |> 
...
s2_agg1 |> group_by(accuracy_level) |> summarize(n=n())
...
cor.test(s2_agg1$els, s2_agg1$log_abs_error)
...
library(lme4)
mixed_model <- lmer(abs_error ~ els + (1 | id), data = s2_agg1)
...
polr_model_ref_interaction <- MASS::polr(accuracy_level ~ els * refClass + calc, data = s2_agg1)
summary(polr_model_ref_interaction)

# Example for refClass = USD, calc = No Calculator
new_data <- expand.grid(els = seq(min(s2_agg1$els), max(s2_agg1$els), length.out = 100),
                        refClass = "USD",
                        calc = "No Calculator")
