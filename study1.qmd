...
k <- sum1 |> group_by(id) |>  summarize(total_kWh=sum(total_kWh),orig_kWh=sum(orig_kWh),pct_change=round((orig_kWh-total_kWh)/orig_kWh,3)) |> 
  mutate(matched_goal = (pct_change == .15))
...
sum1 <- sum1 |> filter(!id %in% outlier_id)
